  ...
  for (const alert of (alerts ?? []) as AlertRow[]) {
    ...
    try {
      ...
      const { data: profile } = await supabase
        .from('profiles').select('email,tier').eq('id', alert.user_id).maybeSingle();
      if (!profile?.email) continue;

      // Paid accounts only.
      //
      // Every alert runs a full search, so the email is free but finding the
      // jobs is not. The first version billed us for anyone who had ever turned
      // alerts on, including accounts that stopped opening the app months ago.
      // Tying it to a live subscription means the cost sits against revenue
      // rather than against a dormant free signup.
      if (profile.tier !== 'pro') continue;
