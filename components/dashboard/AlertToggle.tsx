...
// the previous one.
export default function AlertToggle({ options, tier }: {
  options: ScrapeOptions | null;
  tier: 'free' | 'pro';
}) {
  // Paid only: each alert runs a full search every morning, so this is a
  // recurring cost per subscriber. Offering it to free accounts would bill us
  // monthly for people who never come back. Hidden rather than shown-and-
  // blocked — a switch that does nothing is worse than no switch.
